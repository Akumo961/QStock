...
id, name (e.g. "Laptop A", "Dell Latitude"), description, item_code (unique),
category (electronics|school_items|decorations|clothes|games|other),
status (available|borrowed|maintenance|retired), quantity (total owned),
available_quantity (currently free to borrow), brand (e.g. Dell, HP, Apple),
model, serial_number, location, is_borrowable, requires_approval,
max_borrow_days, purchase_date (text, not a date type), notes, image_url,
...
id, user_id, item_id, status (borrowed|returned|overdue|cancelled), quantity,
borrowed_at, due_date, returned_at (NULL = not yet returned), purpose, notes,
approved_by_admin_id, condition_at_borrow, condition_at_return, created_at,
updated_at.
A currently-borrowed item usually has status='borrowed' AND returned_at IS NULL.

TABLE requests (PK: id) — user requests/orders (new items, special borrows).
FKs: user_id -> users.id, item_id -> items.id (nullable),
responded_by_admin_id -> users.id (nullable).
id, user_id, item_id, order_type (new_item|special_borrow|other), title,
description, status (pending|approved|rejected|ready), priority (legacy,
usually "normal"), needed_date, ready_date, admin_response,
responded_by_admin_id, responded_at, request_type (legacy), created_at,
updated_at.
"""
...
- current inventory/currently available/available stock -> SUM(available_quantity).
- borrowed now/currently has/who has -> transactions.status IN ('borrowed','overdue') AND returned_at IS NULL.
- overdue -> transactions.status='overdue' OR (due_date < CURRENT_DATE AND returned_at IS NULL).
- most borrowed item -> count/sum transactions grouped by item, excluding cancelled.
- never borrowed -> item with no non-cancelled transaction row (LEFT JOIN, t.id IS NULL).
- low stock (no threshold given) -> available_quantity < 5.
- under maintenance -> status='maintenance'. retired -> status='retired'.
- this month -> borrowed_at/created_at >= date_trunc('month', CURRENT_DATE).
- me/my/I borrowed/current user -> use the Authenticated User block in the user prompt.

Synonyms: laptop/notebook/computer/PC/desktop -> ILIKE-match name, description,
brand, AND model (not just category). Dell/HP/Apple/Lenovo etc. are brands
unless clearly an item name/model. Category mapping: electronics/electronic/
computer gear->electronics; school supplies/items->school_items;
decorations/decor->decorations; clothing/clothes/uniforms->clothes;
games/toys->games.
"""
